class player{ //defining the player
	...
	update(){ //update loop
      ...
      if (input.includes(arrowKeys[i])){
		...
		let cast = directionVectors[this.dir].copy().mult(gameSpeed*1.2) //cast collisions going in the current direction
		move = true//I will move by default
	...
	show(){ //display event
    let  anim = 0;//smooootthhhh animation number that goes back and forth
    let disp = dispCoords(this.pos,true) //get display coords
    
    if(move&&!paused){
      pTicks = ticks;
    }
    if (useSprites && usePacmanSprites){
      anim = 0.1+abs(33*sin(pTicks*10))
      if (this.dead){
        anim = max(((this.deadTicks-60) / 120),0) * 10
        image(sprites.pacdeath[round(anim)], disp.x, disp.y, CELL, CELL)
      }else{
        ...
        image(sprite, disp.x, disp.y, CELL, CELL)
      }
      return
    }
    anim = abs(44*sin(pTicks*10))
		noStroke() //no line needed
		fill(0xff,0xff,0) //fill full yellow
		let rot = this.dir*-90 //turn direction into rotation
    
		if (this.dead){anim = constrain((100+(+this.deadTicks-60)*2), 0, 179)} //if im dead make it keep going up instead
    if(anim>=170){
      image(dieEndImg,disp.x,disp.y,CELL,CELL)
      this.deadTicks++;
    }else{
      arc(disp.x,disp.y,ceil(CELL*(7/8)),ceil(CELL*(7/8)),anim+rot,(anim*-1)+rot) //using an arc to represent pacman
    }
