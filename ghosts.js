...
class ghost{
	...
	update(){ //mainloop of ghost behavior
		...
		for (let i of ghosts){ //loop through ghosts
			if (i == this){continue} //don't count me
		...
		if (this.pen){// am I in the pen?
			switch (this.type){ // go through the ghosts
				case 1: //pinky exits at 310 dots remaining
					if (dots.length <= 190){this.exitPen();otherExit=true;}
				break
				case 2: //inky exits at 190
					if (dots.length <= 100 && !otherExit){setTimeout(this.exitPen(),10000)}
				break
				case 3: //clyde exits at 106
					if (dots.length <= 50 && !otherExit){this.exitPen()}
				break
			} //blinky = skipped because he spawns outside the pen
			return null
		}
    
		if (ticks % 2 == 0){ //skip this unless this is the 10th tick for better performance.
    ...
    if (useSprites){
      if ((fright <= 0 || this.eaten == true) && !this.eyes){
        image(this.sprite[this.dir*2 + (ticks % 40 < 20 ? 1 : 0)],disp.x,disp.y,CELL,CELL)
      } else if (fright > 0 && !this.eaten && !this.eyes){
        image(sprites.fright[ticks % 60 <= 30 && fright <= 180 ? 2 : 0 + ticks % 30 < 15 ? 1 : 0],disp.x, disp.y, CELL, CELL)
      }else if (this.eyes){
        image(sprites.eyes[this.dir],disp.x, disp.y, CELL, CELL)
      }
